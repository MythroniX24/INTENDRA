...
/**
 * AiOrchestrator — routes requests between local and cloud AI.
 *
 * UPGRADE: Added cloudConsentCallback so the ViewModel can ask the user
 * for permission before sending data to cloud in HYBRID mode.
 * This replaces the silent auto-escalation that was a privacy risk.
 */
class AiOrchestrator(
    ...
    suspend fun process(
        ...
        val result: AiEngineResult = when (routingDecision) {
            ...
            RoutingDecision.CLOUD -> runCloud(userInput, memory)

            RoutingDecision.LOCAL_WITH_CLOUD_FALLBACK -> {
                val localResult = runLocal(userInput, memory)
                if (isGoodResult(localResult.intentJson)) {
                    localResult
                } else if (isInternetAvailable() && cloudEngine.isConfigured()) {
                    // PRIVACY FIX: ask for consent before cloud escalation in HYBRID mode
                    var userAllowed = false
                    onCloudConsentNeeded { allowed -> userAllowed = allowed }
                    if (userAllowed) {
                        try { cloudEngine.parseIntent(userInput, memory, chatHistory).copy(source = AiSource.FALLBACK) }
