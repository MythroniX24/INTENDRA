...
class GitPlugin(context: Context) : IPlugin {
    ...
    private fun dir(args: Map<String, String>) = ShellArgumentPolicy.shellQuote(args["workdir"] ?: "/sdcard")
    ...
    private suspend fun push(args: Map<String, String>): PluginResult {
        val d = dir(args); val msg = ShellArgumentPolicy.shellQuote(args["message"] ?: "update"); val branch = ShellArgumentPolicy.shellQuote(args["branch"] ?: "HEAD")
        return shell("cd $d && git add -A 2>&1 && git commit --allow-empty -m $msg 2>&1 && git push origin $branch 2>&1", 120_000L)
    }
    private suspend fun pull(args: Map<String, String>) = shell("cd ${dir(args)} && git pull origin ${ShellArgumentPolicy.shellQuote(args["branch"] ?: "HEAD")} 2>&1", 120_000L)
    private suspend fun clone(args: Map<String, String>): PluginResult {
        ...
        return shell("cd ${ShellArgumentPolicy.shellQuote(args["dir"] ?: "/sdcard")} && git clone ${ShellArgumentPolicy.shellQuote(url)} 2>&1", 180_000L)
    }
    private suspend fun init(args: Map<String, String>): PluginResult {
        val d = dir(args); val name = ShellArgumentPolicy.shellQuote(args["name"] ?: "myproject")
    ...
    private suspend fun commit(args: Map<String, String>): PluginResult {
        val d = dir(args); val msg = ShellArgumentPolicy.shellQuote(args["message"] ?: "update")
        return shell("cd $d && git add -A 2>&1 && git commit -m $msg 2>&1")
    }
    private suspend fun log(args: Map<String, String>) = shell("cd ${dir(args)} && git log --oneline -${ShellArgumentPolicy.shellQuote(args["count"] ?: "20")} 2>&1")
    ...
    private suspend fun checkout(args: Map<String, String>): PluginResult {
        ...
        return shell("cd ${dir(args)} && git checkout ${ShellArgumentPolicy.shellQuote(branch)} 2>&1")
    }
    private suspend fun diff(args: Map<String, String>) = shell("cd ${dir(args)} && git diff 2>&1 | head -80")
    private suspend fun stash(args: Map<String, String>) = shell("cd ${dir(args)} && git stash ${ShellArgumentPolicy.shellQuote(args["action"] ?: "push")} 2>&1")
    private suspend fun authSetup(args: Map<String, String>): PluginResult {
        ...
        val r = shell("git config --global user.name ${ShellArgumentPolicy.shellQuote(username)} && git config --global user.email ${ShellArgumentPolicy.shellQuote(email)}")
    ...
    private suspend fun config(args: Map<String, String>): PluginResult {
        ...
        return if (value != null) shell("cd ${dir(args)} && git config ${ShellArgumentPolicy.shellQuote(key)} ${ShellArgumentPolicy.shellQuote(value)} 2>&1")
        else shell("cd ${dir(args)} && git config ${ShellArgumentPolicy.shellQuote(key)} 2>&1")
    }
    private suspend fun sshKey(args: Map<String, String>): PluginResult {
        val email = args["email"] ?: "interndra@local"
        val name = args["name"]?.takeIf { it.matches(Regex("[A-Za-z0-9._-]{1,64}")) } ?: "id_ed25519"
        // The filename is restricted to safe identifier characters, so leave
        // the tilde unquoted to preserve shell home-directory expansion.
        val keyPath = "~/.ssh/$name"
        val pubKeyPath = "~/.ssh/$name.pub"
        val check = shell("test -f $keyPath -o -f $pubKeyPath 2>&1")
        if (check.success) {
            val pub = shell("cat $pubKeyPath 2>&1")
            return PluginResult(true, "SSH key exists at ~/.ssh/$name\n\nPublic key:\n${pub.output}\n\nAdd at: https://github.com/settings/keys")
        }
        // `name` is restricted to a safe filename, so keep the tilde
        // unquoted here; otherwise the shell would not expand $HOME.
        val r = shell("mkdir -p ~/.ssh && chmod 700 ~/.ssh && ssh-keygen -t ed25519 -C ${ShellArgumentPolicy.shellQuote(email)} -f $keyPath -N '' -q 2>&1")
