...
class TermuxPlugin(context: Context) : IPlugin {
    ...
    override suspend fun execute(command: String, args: Map<String, String>) =
        ...
        when (command.removePrefix(CMD_PREFIX)) {
            ...
            "vibrate" -> {
                val duration = args["duration"]?.toLongOrNull()?.coerceIn(1L, 60_000L) ?: 500L
                shell("termux-vibrate -d $duration 2>&1")
            }
            "sensor" -> shell("termux-sensor -s ${ShellArgumentPolicy.shellQuote(args["sensor"] ?: "all")} -n 1 2>&1")
    ...
    private suspend fun runPython(args: Map<String, String>): PluginResult {
        ...
        return shell("python3 -c ${ShellArgumentPolicy.shellQuote(code)} 2>&1")
    }
    private suspend fun pipInstall(args: Map<String, String>): PluginResult {
        ...
        return shell("pip install ${ShellArgumentPolicy.shellQuote(pkg)} 2>&1 || pip3 install ${ShellArgumentPolicy.shellQuote(pkg)} 2>&1", 120_000L)
    }
