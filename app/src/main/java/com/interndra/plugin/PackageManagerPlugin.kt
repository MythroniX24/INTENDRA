...
/**
 * PackageManagerPlugin — package management via built-in shell.
 */
class PackageManagerPlugin(context: Context) : IPlugin {

    companion object { private const val TAG = "PackageManagerPlugin"; private const val CMD_PREFIX = "pkg:" }

    override val id = "pkgmgr"; override val name = "Package Manager"
    override val description = "Unified interface for pkg/pip/npm via built-in shell"
    ...
    private suspend fun detect(): PluginResult {
        ...
        for ((name, cmd) in checks) {
            val r = exec("which $name 2>/dev/null | head -1")
            if (r.success && r.output.isNotBlank()) {
                val ver = exec("$name --version 2>&1 | head -1")
    ...
    private suspend fun install(args: Map<String, String>): PluginResult {
        ...
        val cmd = when (type) { "pip" -> "pip install $name 2>&1 || pip3 install $name 2>&1"; "npm" -> "npm install $name 2>&1"; else -> "pkg install -y $name 2>&1" }
        return exec(cmd, 120_000L)
    }
    private suspend fun update() = exec("pkg update -y 2>&1", 120_000L)
    private suspend fun upgrade(args: Map<String, String>) = exec(if (args["name"] == "all") "pkg upgrade -y 2>&1" else "pkg install -y ${args["name"] ?: "all"} 2>&1", 180_000L)
    private suspend fun list(args: Map<String, String>): PluginResult {
        val sb = StringBuilder()
        if ((args["type"] ?: "all") in listOf("all","pkg")) { val r = exec("pkg list-installed 2>&1 | head -40"); if (r.success) sb.appendLine("📦 pkg:\n${r.output.take(500)}\n") }
        if ((args["type"] ?: "all") in listOf("all","pip")) { val r = exec("pip list 2>&1 | head -30 || pip3 list 2>&1 | head -30"); if (r.success) sb.appendLine("🐍 pip:\n${r.output.take(500)}\n") }
        return PluginResult(true, sb.toString().ifEmpty { "No packages found" })
    }
    private suspend fun search(args: Map<String, String>) = exec("pkg search ${args["query"] ?: return PluginResult(false, "", error = "Missing 'query'")} 2>&1 | head -30")
    private suspend fun uninstall(args: Map<String, String>): PluginResult {
        val name = args["name"] ?: return PluginResult(false, "", error = "Missing 'name'")
        return exec(when (args["type"] ?: "pkg") { "pip" -> "pip uninstall -y $name 2>&1 || pip3 uninstall -y $name 2>&1"; "npm" -> "npm uninstall $name 2>&1"; else -> "pkg uninstall $name 2>&1" })
    }
    private suspend fun info(args: Map<String, String>): PluginResult {
        val name = args["name"] ?: return PluginResult(false, "", error = "Missing 'name'")
        val r = exec("pkg show $name 2>&1 | head -30")
        return if (r.success && !r.error.contains("not found")) r else exec("pip show $name 2>&1 || pip3 show $name 2>&1")
    }
